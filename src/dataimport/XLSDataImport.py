...
class XLSDataImport(DataImport):
    ...
    :type worksheet: str
    :param range: the range of cells to import into a table (attempts to detect
        a usable range of values if not provided)
    :type range: :class:`.CellRange`
        ...
        for row in range(firstRow, lastRow):
            ...
            for col in range(firstCol, lastCol):
                ...
                if isinstance(value, basestring):
                    ...
                    if not value.strip():
                        value = None
                elif self.getTypes()[len(rowValues)] in ("int", "float"):
